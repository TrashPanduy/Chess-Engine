...
//initialize commandBuffer and ready it for use via class var commandBuffer.
VkCommandBuffer VulkanHelper::beginSingleTimeCommands() {
    ...
    //returns initiallized comman buffer, ready to take in commands.
    return commandBuffer;
}

//empty commandBuffer then deleto dorito
...
void VulkanHelper::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
bool VulkanHelper::TestRayOBBIntersection(glm::vec3 rayOrigin, glm::vec3 rayDir, glm::vec3 aabbMin, glm::vec3 aabbMax, glm::mat4 modelMatrix, float& outDistance)
{

    float tMin = 0.0f;
    float tMax = 1000.0f;

    glm::vec3 obbPosWorld = glm::vec3(modelMatrix[3]);
    glm::vec3 delta = obbPosWorld - rayOrigin;

    // X, Y, Z axis of OBB from model matrix
    for (int i = 0; i < 3; i++) {
        glm::vec3 axis = glm::normalize(glm::vec3(modelMatrix[i]));
...
bool VulkanHelper::RayIntersectsMesh(const glm::vec3& rayOrigin,  
...
bool VulkanHelper::RayIntersectsTriangle(const glm::vec3& rayOrigin,
    const glm::vec3& rayDir,
    const glm::vec3& v0,
    const glm::vec3& v1,
    const glm::vec3& v2,
